    ...
    if (isset($_POST['name'])) {
      if (isset($_POST['agree'])) {
        ...
        if ($password == $c_password) {
          $password = md5('d_engr_web' . $password);

          $insert_user_admin = mysqli_query($con, "SELECT * FROM `suscription` WHERE `email`='$user_type'");
          $row_user_admin = mysqli_num_rows($insert_user_admin);


          if ($row_user_admin == 1) {
            //echo $row_user_admin;
            $insert_user_admin = mysqli_query($con, "UPDATE  `suscription` SET `name`= '$name' ,`phone`= '$phone', `password`='$password' WHERE `email`= '$user_type'");
