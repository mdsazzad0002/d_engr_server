...
if (isset($_POST['user_type'])) {
	...
	if (isset($_FILES['profile_pic'])) {
		...
		$s_r_pic = mysqli_fetch_assoc($con->query("SELECT * FROM `suscription` WHERE `email`='$user_type'"))['file'];
		if (!empty($s_r_pic)) {
			if (file_exists('../../assets/images/' . $s_r_pic)) {
				unlink('../../assets/images/' . $s_r_pic);
			}
		}


		$update = $con->query("UPDATE `suscription` SET `file`='$file_name' WHERE `email` ='$user_type'");
		if ($update) {
			$move = move_uploaded_file($tmp_name, '../../assets/images/' . $file_name);
			if ($move) {
